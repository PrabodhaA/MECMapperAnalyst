...
public class AnalyseFunction
{
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "chat")] HttpRequestData req)
    {
        ...
        if (chatRequest is null) return await ErrorResponse(req, "Invalid request body.", HttpStatusCode.BadRequest);
        if (string.IsNullOrWhiteSpace(chatRequest.Message)) return await ErrorResponse(req, "Message is required.", HttpStatusCode.BadRequest);
        ...
        if (!string.IsNullOrWhiteSpace(mapXml))
        {
            ...
            {
                Role = RoleType.User,
                Content = new List<ContentBase> { new TextContent { Text = $"I am uploading a MEC mapper XML file for analysis.\n\n<mapper_xml>\n{processedXml}\n</mapper_xml>\n\nPlease confirm receipt: tell me the map name, version, and one sentence summary. Then wait for questions." } }
            });
            messages.Add(new Message
            {
                Role = RoleType.Assistant,
                Content = new List<ContentBase> { new TextContent { Text = chatRequest.MapSummary ?? "Map received. Ready for your questions." } }
            });
        }
        ...
        try
        {
            ...
            return await JsonResponse(req, new ChatResponse { Reply = replyText, InputTokens = response.Usage?.InputTokens ?? 0, OutputTokens = response.Usage?.OutputTokens ?? 0 });
        }
        catch (Exception ex)
        {
    ...
    private static async Task<(string? MapXml, string? RequestJson)> ParseMultipartFormAsync(HttpRequestData req)
    {
        ...
        try
        {
            ...
            var boundary = "--" + ct[(bi + 9)..].Trim();
            foreach (var part in body.Split(boundary, StringSplitOptions.RemoveEmptyEntries))
            {
                var ci = part.IndexOf("\r\n\r\n", StringComparison.Ordinal);
                if (ci < 0) continue;
                var content = part[(ci + 4)..].TrimEnd('\r', '\n', '-');
...
public class CorsFunction
{
    ...
    public HttpResponseData Options([HttpTrigger(AuthorizationLevel.Anonymous, "options", Route = "{*any}")] HttpRequestData req)
    {
        var res = req.CreateResponse(HttpStatusCode.OK);
        res.Headers.Add("Access-Control-Allow-Origin", "*");
...
public class ChatRequest { public string Message { get; set; } = ""; public string? MapSummary { get; set; } public List<ConversationTurn>? History { get; set; } }
public class ConversationTurn { public string Role { get; set; } = ""; public string Content { get; set; } = ""; }
public class ChatResponse { public string Reply { get; set; } = ""; public int InputTokens { get; set; } public int OutputTokens { get; set; } }
